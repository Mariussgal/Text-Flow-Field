...
function createExplosion() {
    ...
    const velocityMagnitude = Math.sqrt(mouseVelocityX * mouseVelocityX + mouseVelocityY * mouseVelocityY);
    const velocityAngle = Math.atan2(mouseVelocityY, mouseVelocityX);
    ...
    for (let i = 0; i < CONFIG.EXPLOSION_PARTICLES; i++) {
        ...
        if (velocityMagnitude > 5 && Math.random() < 0.7) {
            ...
            angle = velocityAngle + (Math.random() - 0.5) * Math.PI * 0.8;
            speed = 5 + Math.random() * 15 + velocityMagnitude * 0.8;
        } else {
